...
const getAgendamentosAssistente = async (req, res) => {
...
const getAgendamento = async (req, res) => {
  ...
  try {
    const agendamento = await Agendamento.findById(id).populate(
      "pacienteId",
      "nome"
    );
    ...
    if (!agendamento) {
      return res
        .status(404)
        .json({ error: "O agendamento não foi encontrado" });
    }
...
const createAgendamento = async (req, res) => {
  ...
  if (emptyFields.length > 0) {
    return res
      .status(400)
      .json({ error: "Por favor, preencha todos os dados", emptyFields });
  }
...
const createAgendamentoAssistente = async (req, res) => {
  const { title, start, end, description, tipo, pacienteId, medicoId } =
    req.body;
  ...
  const user = await User.findById(medicoId);
...
const associatePacienteToAgendamento = async (req, res) => {
  ...
  if (
    !mongoose.Types.ObjectId.isValid(agendamentoId) ||
    !mongoose.Types.ObjectId.isValid(pacienteId)
  ) {
    return res
      .status(400)
      .json({ error: "ID de agendamento ou paciente inválido" });
  }
